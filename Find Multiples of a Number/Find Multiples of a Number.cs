...
it should be included as well. There will only ever be positive integers passed into the function, not consisting of 0. 
...
public class Kata
{
  ...
  {
    return Enumerable.Range(0, limit / integer).Select(x => x * integer + integer).ToList();
  }
}